...
/**
 * Comprehensive test class for DistMult knowledge graph embedding model.
 *
 * Tests the DistMult model's initialization, forward pass, scoring with bilinear diagonal product,
...
class DistMultTest {
    ...
    @Test
    fun testDistMultModelInitialization() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
        ...
        assertEquals(dim, entities.shape[1], "Entity embeddings should have dim")

        val edges = distMult.getEdges()
        assertNotNull(edges)
        assertEquals(numEdges, edges.shape[0])
        assertEquals(dim, edges.shape[1], "Relation embeddings should have dim")
    }

    @Test
    fun testDistMultForwardPass() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
    ...
    @Test
    fun testDistMultWithMultipleTriples() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
    ...
    fun testDistMultModelFunction() {
        ...
        val score = distMult.model(testTriple, entities, edges)
        assertNotNull(score)
    ...
    @Test
    fun testDistMultScoreConsistency() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
    ...
    @Test
    fun testDistMultGetOutputShapes() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L

        val distMult = DistMult(numEntities, numEdges, dim)

        // Test with single input
        val inputShapes = arrayOf(Shape(9)) // 3 triples * 3 values each
        val outputShapes = distMult.getOutputShapes(inputShapes)
        ...
        val outputShapesDouble = distMult.getOutputShapes(inputShapesDouble)
    ...
    @Test
    fun testDistMultWithDifferentEntityIndices() {
        val numEntities = 50L
        val numEdges = 5L
        val dim = 16L
    ...
    @Test
    fun testDistMultResourceCleanup() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
    ...
    @Test
    fun testDistMultBatchSizeIndependence() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
        ...
        val score1 = distMult.model(testTriple1, entities, edges)
        val value1 = score1.toFloatArray()[0]

        val testTriple2 = manager.create(longArrayOf(0, 0, 1, 1, 1, 2))
        val score2 = distMult.model(testTriple2, entities, edges)
    ...
    @Test
    fun testDistMultWithSmallDimension() {
        val numEntities = 5L
        val numEdges = 2L
        val dim = 4L
    ...
    @Test
    fun testDistMultWithLargeDimension() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 256L
        ...
        val entities = distMult.getEntities()
        assertEquals(dim, entities.shape[1])
    ...
    @Test
    fun testDistMultBilinearProductProperty() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 8L
        ...
        assertEquals(
            score1.toFloatArray()[0],
            score2.toFloatArray()[0],
            1e-4f,
            "DistMult should be symmetric",
        )
    ...
    @Test
    fun testDistMultWithInitializerBounds() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
    ...
    @Test
    fun testDistMultModelWithSingleEntity() {
        val numEntities = 1L
        val numEdges = 1L
        val dim = 8L
    ...
    @Test
    fun testDistMultForwardInternalWithNegatives() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
    ...
    @Test
    fun testDistMultScoreProperties() {
        val numEntities = 20L
        val numEdges = 5L
        val dim = 16L
        ...
        for (i in 0 until numTestTriples) {
            val iLong = i.toLong()
            triples[i * 3] = iLong % numEntities
            triples[i * 3 + 1] = iLong % numEdges
            triples[i * 3 + 2] = (iLong + 1) % numEntities
        }
    ...
    @Test
    fun testDistMultDiagonalMatrixProperty() {
        val numEntities = 10L
        val numEdges = 3L
        val dim = 16L
        ...
        // DistMult uses diagonal matrices, so entity and edge embeddings have the same dimension
        assertEquals(entities.shape[1], edges.shape[1], "Entity and edge dimensions should match for DistMult")
    }
}
